...
window.PokedexPokeeditPanel = PokedexResultPanel.extend({
    initialize: function(id) {
        ...
        buf += '<dt>Types:</dt> <dd>';
        for(var i = 0; i < pokemon.types.length; i++)buf += `<a href="#" class="type ${toID(pokemon.types[i])} edit-type" data-slot="${i}" style="cursor:pointer;">${pokemon.types[i]}</a> `;
        // Add button to add a second type if only one type exists
        if (pokemon.types.length === 1) buf += `<button class="button add-type" style="font-size:11px;padding:2px 8px;margin-left:8px;">+ Add Type</button>`;
        buf += '</dd>';
        buf += '</dl>';
    ...
    renderFullLearnset: function() {
        ...
        for (let learn of learnset){
            ...
            if (learn.how === 'lvl') {
                ...
                moveRow = moveRow.replace('</a></li>', '</a> <button class="button remove-move" data-index="' + moveIndex + '" style="font-size:11px;padding:2px 6px;margin-left:8px;float:right;">\xd7</button></li>');
                buf += moveRow;
    ...
    updateMoveLevel: function(e) {
        ...
        if (!isNaN(newLevel) && newLevel >= 0) {
            ...
            learnset.sort(function(a, b) {
                ...
                if (a.how === 'lvl' && b.how === 'lvl') return a.level - b.level;
                return 0;
            });
    ...
    updateStat: function(e) {
        ...
        if (!isNaN(newValue) && newValue >= 1 && newValue <= 255) {
            ...
            if (color > 360) color = 360;
            this.$('.statbar-fill[data-stat="' + stat + '"]').css('width', width + 'px').css('background', 'hsl(' + color + ',85%,45%)').css('border-color', 'hsl(' + color + ',75%,35%)');
            // Recalculate BST
            var bst = 0;
            for(var s in BattleStatNames)bst += pokemon.baseStats[s];
    ...
    editType: function(e) {
        ...
        if (newType && newType.trim()) {
            ...
            } else alert('Invalid type. Please choose from: ' + types.join(', '));
        }
    },
    addType: function(e) {
        e.preventDefault();
        ...
        if (pokemon.types.length >= 2) {
            alert("Pok\xe9mon can only have up to 2 types.");
            return;
        }
        var types = [
            'Normal',
