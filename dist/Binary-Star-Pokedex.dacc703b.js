...
window.PokedexLocationsPanel = PokedexResultPanel.extend({
    initialize: function() {
        ...
        buf += '</ul>';
        buf += '<div class="searchboxwrapper"><input class="textbox searchbox" type="search" name="q" value="" autocomplete="off" placeholder="Search locations by name, Pok\xe9mon, item, or trainer..." /></div>';
        buf += '</form>';
    ...
    handleSearch: function(e) {
        ...
        if (!query) this.filteredLocations = this.allLocations;
        else this.filteredLocations = this.allLocations.filter(function(loc) {
            if (!loc) return false;
            ...
            if (loc.encounters) for(var i = 0; i < loc.encounters.length; i++){
                var encounter = loc.encounters[i];
                if (encounter.pokemon) for(var j = 0; j < encounter.pokemon.length; j++){
                    ...
                    if (mon.sos && Array.isArray(mon.sos)) for(var k = 0; k < mon.sos.length; k++){
            ...
            // Search by items
            if (loc.items) for(var i = 0; i < loc.items.length; i++){
                if ((loc.items[i].item || '').toLowerCase().indexOf(query) >= 0) return true;
            }
            // Search by shop items
            if (loc.shops) for(var i = 0; i < loc.shops.length; i++){
                if ((loc.shops[i].item || '').toLowerCase().indexOf(query) >= 0) return true;
            }
            // Search by trainers
            if (loc.trainers) for(var i = 0; i < loc.trainers.length; i++){
                var tid = loc.trainers[i];
                var trainer = (window.Trainers || []).find(function(t) {
                    return t.id === tid;
                });
                if (trainer && (trainer.name || '').toLowerCase().indexOf(query) >= 0) return true;
            }
            if (loc.bossTrainers) for(var i = 0; i < loc.bossTrainers.length; i++){
