...
window.PokedexTrainerPanel = PokedexResultPanel.extend({
    initialize: function(id) {
        ...
        buf += "<style>.dexentry .abilitydesccol { white-space: normal !important; overflow: visible !important; width: auto !important; height: auto !important; max-width: none !important; float: none !important; display: inline !important; }.dexentry .movedesccol { white-space: normal !important; overflow: visible !important; width: auto !important; height: auto !important; max-width: none !important; float: none !important; display: inline !important; }.dexentry .namecol { float: none !important; display: inline !important; padding-top: 0 !important; height: auto !important; }.trainer-sprite-bg { position: absolute; right: 0; top: 32px; width: 256px; height: 128px; transform: scale(0.5); transform-origin: top right; opacity: 1; pointer-events: none; z-index: 0; }.dexentry h1 { margin-top: 0; margin-bottom: 6px; white-space: nowrap; position: relative; z-index: 1; }.dexentry h1 a { display:inline-block; white-space:nowrap; vertical-align: middle; }.dexentry > * { position: relative; z-index: 1; }</style>";
        buf += '<div class="trainer-sprite-bg" style="' + trainerSprite + '"></div>';
        ...
        if (trainerLocation) buf += '<dt>Location:</dt> <dd><a href="' + Config.baseurl + 'locations/' + trainerLocation.id + '" data-target="push">' + escapeHTML(trainerLocation.name) + '</a></dd>';
        else buf += '<dt>Location:</dt> <dd><a href="' + Config.baseurl + 'locations/" data-target="push">Coming soon</a></dd>';
        ...
        if (typeof notes === 'string' && notes.trim().length) buf += '<dt>Extra Notes:</dt> <dd>' + escapeHTML(notes) + '</dd>';
        buf += '</dl>';
        // Team
        buf += '<h3>Team</h3>';
        ...
        var isIllegal = function(pokemon, speciesData) {
            ...
            if (pokemon.ability) {
                ...
                for(var slot in speciesData.abilities)if (speciesData.abilities[slot] === pokemon.ability) {
                    abilityLegal = true;
                    break;
                }
            ...
            if (pokemon.moves && pokemon.moves.length > 0) {
                ...
                for(var j = 0; j < pokemon.moves.length; j++){
                    ...
                    var moveLegal = false;
                    for(var k = 0; k < speciesLearnset.length; k++)if (toID(speciesLearnset[k].move) === moveID) {
                        moveLegal = true;
                        break;
                    }
        ...
        for(var i = 0; i < (trainer.team || []).length; i++){
            ...
            var spritesBlock = '';
            if (monData) spritesBlock += '<a href="' + Config.baseurl + 'pokemon/' + monID + '" data-target="push" title="' + escapeHTML(monData.name) + '">' + '<span class="picon" style="' + getPokemonIcon(monID) + ';display:inline-block;vertical-align:middle"></span>' + '</a>';
            else spritesBlock += '<span class="picon" style="' + getPokemonIcon(monID) + ';display:inline-block;vertical-align:middle"></span>';
            if (m.item) {
            ...
            if (types.length) buf += '<div class="resultsub" style="margin-top:2px;margin-left:0">' + types.map(function(t) {
            ...
            if (m.ability) {
                ...
                if (abilityObj) {
                    var abilRow = window.BattleSearch && typeof BattleSearch.renderAbilityRow === 'function' ? BattleSearch.renderAbilityRow(abilityObj) : '<li class="result" style="background:transparent"><a href="' + Config.baseurl + 'abilities/' + abilID + '" data-target="push">' + '<span class="col namecol">' + escapeHTML(abilityObj.name) + '</span> ' + '<span class="col abilitydesccol">' + escapeHTML(abilityObj.shortDesc || abilityObj.desc || '') + '</span> ' + '</a>' + '</li>';
            ...
            if (m.nature) {
                ...
                if (eff) natureHtml = '<small>Nature:</small> ' + escapeHTML(m.nature) + ' (<span style="color:#1f9d3a">' + eff[0] + '\u2191</span> / <span style="color:#c22e28">' + eff[1] + "\u2193</span>)";
