...
(function (exports, $) {
	...
	function getAbilityUseCount(id) {
		if (!__abilityUseCountCache) {
			...
			for (var pokeId in BattlePokedex) {
				...
				seen.forEach(aid => { __abilityUseCountCache[aid] = (__abilityUseCountCache[aid] || 0) + 1; });
			}
		}
	...
	// Cache Pokemon wild/trainer usage counts
			...
			if (window.Locations) {
				for (let loc of window.Locations) {
					if (loc.pokemon && Array.isArray(loc.pokemon)) {
						for (let encounter of loc.pokemon) {
							if (encounter.name) {
								const pokemonId = toID(encounter.name);
						...
						for (let pokemon of trainer.team) {
							if (pokemon.name) {
								const dispName = typeof window.translateDisplayName === 'function' ? 
									window.translateDisplayName(pokemon.name) : pokemon.name;
	...
	Search.prototype.find = function (query, firstElem) {
		...
		if (this.engine && this.engine.typedSearch && this.engine.typedSearch.searchType === 'ability') {
			...
			if (!this.resultSet.length || this.resultSet[0][0] !== 'sortability') {
				this.resultSet = [['sortability', '']].concat(this.resultSet);
			}
			// Hide zero-user abilities only when no query string (q) present (still show after user searches)
			if (!this.q && !this.filters) {
